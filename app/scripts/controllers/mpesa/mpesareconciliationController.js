        ...
        mpesareconciliationController: function ($q,$http,scope, resourceFactory, location,http,dateFilter,$modal,sessionManager,routeParams,$rootScope) {
           ...
           //alert($rootScope.ofId);
            ...
            if(routeParams.status1!=null){
                ...
                http({
                    method: 'GET',
                    url: 'http://52.19.21.68:9292/caritasmpesa/mpesa/Search?status=UNMP&FromDate='+''+'&ToDate='+scope.toDate+'&officeId='+scope.officeId+'&mobileNo='
                ...
                $http.get("http://52.19.21.68:9292/caritasmpesa/mpesa/getunmappedtransactions?officeId="+scope.officeId).success(function (data) {
                    deferred.resolve(data);
                    scope.completetransaction = data;
                });
            }
            ...
            scope.Mpesasearch=function() {
                ...
                http({
                    method: 'GET',
                    url: 'http://52.19.21.68:9292/caritasmpesa/mpesa/Search?status='+ scope.text+'&FromDate='+ scope.fromDate+'&ToDate='+scope.toDate+'&mobileNo='+scope.searcText+'&officeId='+scope.officeId
                }).success(function (data) {
                    deferred.resolve(data);
                    scope.completetransaction=data;
                });
